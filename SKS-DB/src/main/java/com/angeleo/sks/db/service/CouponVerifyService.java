...
public class CouponVerifyService {
     ...
     * @return
     */
    public SksCoupon checkCoupon(Integer userId, Integer couponId, Integer userCouponId, BigDecimal checkedGoodsPrice, List<SksCart> cartList) {
        ...
        // 检测商品是否符合
        Map<Integer, List<SksCart>> cartMap = new HashMap<>();
        //可使用优惠券的商品或分类
        ...
        if (goodType.equals(CouponConstant.GOODS_TYPE_CATEGORY) ||
            ...
            for (SksCart cart : cartList) {
                ...
                List<SksCart> carts = cartMap.get(key);
                if (carts == null) {
            ...
            for (Integer goodsId : goodsValueList) {
                List<SksCart> carts = cartMap.get(goodsId);
                for (SksCart cart : carts) {
