...
import com.angeleo.sks.db.service.SksKeywordService;
import com.angeleo.sks.db.service.SksSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private SksKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        SksKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<SksKeyword> hotKeywordList = keywordsService.queryHots();

        List<SksSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<SksKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (SksKeyword key : keywordsList) {
