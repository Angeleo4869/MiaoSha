...
import com.angeleo.sks.db.util.*;
...
import com.angeleo.sks.core.notify.NotifyService;
import com.angeleo.sks.core.notify.NotifyType;
...
import com.angeleo.sks.core.util.DateTimeUtil;
import com.angeleo.sks.core.util.JacksonUtil;
import com.angeleo.sks.core.util.ResponseUtil;
import com.angeleo.sks.core.util.IpUtil;
import com.angeleo.sks.wx.task.OrderUnpaidTask;
...
public class WxOrderService {
    ...
    @Autowired
    private SksAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<SksOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (SksOrder o : orderList) {
            ...
            List<SksOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        SksOrder order = orderService.findById(userId, orderId);
        if (null == order) {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            ...
            //秒杀规则已经过期
            if (rules.getStatus().equals(SnapUpConstant.RULE_STATUS_DOWN_EXPIRE)) {
                return ResponseUtil.fail(WxResponseCode.GROUPON_EXPIRED, "秒杀已过期!");
            }
            //秒杀活动已经下线
            if (rules.getStatus().equals(SnapUpConstant.RULE_STATUS_DOWN_ADMIN)) {
        ...
        // 收货地址
        SksAddress checkedAddress = addressService.query(userId, addressId);
        ...
        if (cartId.equals(0)) {
            ...
            SksCart cart = cartService.findById(cartId);
        ...
        if (couponId != 0 && couponId != -1) {
            SksCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice, checkedGoodsList);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (SksCart cartGoods : checkedGoodsList) {
            // 订单商品
            SksOrderGoods orderGoods = new SksOrderGoods();
        ...
        for (SksCart checkGoods : checkedGoodsList) {
            ...
            SksGoodsProduct product = productService.findById(productId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            SksCouponUser couponUser = couponUserService.findById(userCouponId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        SksOrder order = orderService.findBySn(orderSn);
        if (order == null) {
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<SksOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        SksOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        SksOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        SksComment comment = new SksComment();
    ...
    public void releaseCoupon(Integer orderId) {
        List<SksCouponUser> couponUsers = couponUserService.findByOid(orderId);
        for (SksCouponUser couponUser: couponUsers) {
            // 优惠券状态设置为可使用
